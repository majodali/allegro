...
7. **8-byte string literals collide with Int in `typeLiterals`** — `src/runtime.ts:32` types any 64-bit Bits as Int, but UTF-8 strings of exactly 8 bytes are also 64 bits. Result: `"a*b/aaab"` gets IntType and displays as a raw number. Fix: parser should wrap string literals with StringType directly (withType + stringToBits) rather than relying on length-based inference in typeLiterals.
8. **Stack overflow in `seq(zero_or_more(m), m2)` parser combinators** — Composing combinator-style matchers where one is a closure-producing recursive function (like zero_or_more) seems to trigger stack overflow even with tail-recursive helpers. Works standalone but fails when used as an argument to another combinator. Likely related to how Allegro handles captured function-valued variables across closure boundaries. Workaround: direct tree interpretation rather than combinator composition.
9. **Mutual recursion between user functions and parse tree walkers fails silently** — In the regex DSL demo, when `match_base` (handling grouped "(pattern)") mutually recurses with `match_pattern`, subsequent `print` calls on the result silently fail. Works fine for non-recursive interpreters. Symptom: the print returns no output; no error thrown. Likely partial-evaluation of the mutually-recursive call chain produces an unresolved residual that print defers on but never resolves. Investigate how bare-expression residuals of deeply-nested user recursion interact with forward-chaining. (Workaround for demo: avoid recursive grammar in the parse tree interpreter.)
