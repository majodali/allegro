...
- [ ] Provability arc Phase B — refinements as proof substrate. Algebraic propagation (`y = x + 1` inherits `x`'s predicate), relational refinements (`a < b` joint), bounded quantification (`∀ i, 0 ≤ i < xs.length, P(xs[i])`), subtyping on refinements, counterexample generation, pilot rewrite of `lib/math.alg` with annotated signatures.
- [ ] Provability arc Phase C — invariants as first-class pervasive syntax (`invariant`, `requires`, `ensures`, `assumes` at any binding site).
