...
- **T-R6** — Divergence-aware inlining cutoff is the accepted design for the measured precompile pathology (a binding whose tier is `undischarged`/`partial` is never PE-inlined; ~43s for one compile measured at B-028 F4); implementation deferred to the next analyzer/perf pass with B-087 — accepted (deferred) — reasoning: `docs/design/standard/totality.md` §7 + §8
