...
| 53 | **A constant is rebuilt in a loop**: `wrapAsUntypedFunction` is deterministic per primitive, and both Layer-1 builders call it for every primitive on every scope build — 354 calls over 177 distinct primitives for one file. 90% of all repeat attachments are this one path | **B-122** |
