...
| `entry-sequence-composite.md` | B-120: a Structure becomes an ordered sequence of optionally-keyed entries; the map becomes an index below the specification (D48(a)) | **closed → B-120** | All six chunks landed 2026-09, suite 1202/1202. `dense`, `materializeView`, `__length`, `isDense`, **W6** and `isMetaSlotKey` all deleted; `concepts.md` §16/§17 retired in place and IC-2 marked EXECUTED. Two places execution corrected the ruling: the index policy splits by SIZE not by role (93.4% of 4.24M lookups served by a scan of 3.63 entries), and an empty array needed one host bit the ruling did not anticipate — which opened the host-plane audit (B-133, B-135). Residue: E5's unenforced namespace property → B-104; `Structure.positional` → B-133 |
...
| `host-plane-declaration.md` | B-135/B-137: declare the host plane from the slot registry — generated interfaces and accessors, one way in, a lint that can see the plane | **draft** | Awaiting the §6 rulings, chiefly *declare on the value interfaces or on a separate host-plane interface reached only through the accessor layer*. Census re-run on implementation files only: **170** `as any` property accesses over **41** properties — 81 reach a REGISTERED property whose only missing piece is its TYPE, and ~46 reach ~13 genuinely undeclared L0 host properties. The registry lacks two fields (the host type; the value's type) and generation is mechanical once they exist — the emitted-file mechanism was probed and typechecks, including the `import type` cycles |
| `plane-interfaces.md` | B-112: the four owed plane hooks — the base holds the symbol, the layer installs the meaning | **draft** | §7 rulings taken 2026-09; chunk sequence still owed. Measured first: delta 23's *27 upward symbols* is **131 imported / 98 distinct**, and only **4** are checkers — so the four named interfaces cover 4 of 98 and §6 files the other five populations rather than working them. Carries six PROVISIONAL principles (§3) with a stated re-analysis trigger (§3.1): re-run B-121 C2's survey after this lands. P6 (a shared type is a shared representation) is accepted with a recorded tension — the semantic hierarchy that would dissolve it is M9 |
