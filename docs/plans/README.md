...
| `metadata-on-values.md` | B-121: delete the carrier — metadata becomes a field on every representation kind, and values that will carry it are built with it (D48(b)(c)) | **draft** | Awaiting ratification of §6 (four rulings: names, Symbol interning, W1/W5 retirement, `dataOf` chunk scope). Ruled at B-108 on measurement: 74% of all structures were carriers, 98.5% holding one field. Four probes run before writing — notably, retyping `withMetadata` to return `Value` typechecks with **0 errors**, and `newCarrierStructure` has exactly 2 callers, both inside `withMetadata` |
