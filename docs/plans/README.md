...
| `entry-sequence-composite.md` | B-120: a Structure becomes an ordered sequence of optionally-keyed entries; the map becomes an index below the specification (D48(a)) | **active** | §6 ruled 2026-09-01 — all five recommendations accepted: `entries` is authoritative, duplicate keys are not legal, the index covers scopes only, E1 lands as its own PR, and B-127 precedes the call-site migration. Five probes run before writing — notably **2254 of 2540** record structures have a `bindings` map and a `bindingList` that hold *different objects* for the same slot, and **0** of 166 dense structures ever materialize the legacy view the W6 invariant guards |
