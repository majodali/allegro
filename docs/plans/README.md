...
| `parallel-lanes-process-delta.md` | Parallel-lane working model — the two Tier-0 PROCESS sentences the lane model depends on (§3 pre-ratified chunk sequences, §7 lanes) | **proposal** | Lane model itself landed in `docs/backlog.md` §"Parallel lanes" (Tier 2). This doc holds ONLY the Tier-0 delta, which agents never land (PROCESS §7) — awaiting maintainer sign-off; becomes a one-commit edit on approval, then closes |
