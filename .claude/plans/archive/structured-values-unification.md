...
> Status: **design complete (D1–D46) — promotion in progress.**
...
| D45 | **One construction surface — `define` uniform at every meta-level; Interface and Refinement become kinds; the constructor model (maintainer-ratified 2026-08).** Extends D40+D44. **(1) The composition operation IS the kind recipe's instance construction for Type** — there is no separate `define_kind`; `define` is ONE implementation reading its policy from the kind it is called on (member-transparent kind dispatch: kinds hold Type's drawn `define` symbol; `self` = the kind being instantiated supplies constructor authority, markers, order, operators, laws). Defining a type, an interface, a refinement, or a new kind is the same act on a different kind. **(2) Interface and Refinement are kinds, related to Type by D44's own relations** — the type system's kinds are organized by the type system's relations: **Interface = a REFINEMENT of Type** (restriction: instances are declaration-only — no constructor authority; no new structure ⇒ knowledge layer); **Refinement = a SUB-KIND of Type** (extension: draws Type's kind-member symbols and ADDS instance-members `refines` + `constraints` ⇒ composition/conformance). **(3) Instance-of = shape CONFORMS TO kind, not shape identity** — required for the tower: Interface's shape is Refinement, yet `Interface instanceof Type` holds because Refinement drew Type's kind-member symbols. **The half-lotus matrix** (a C6.1b battery test): `Type : Type` ✓ (D7 fixed point); `Refinement : Type` ✓; `Interface : Refinement : Type` ✓; `Refinement : Interface` ✗ (Refinement holds constructor authority — not declaration-only). **(4) Fluent-API disposition** (removed decisively per the D44 ruling — no sugar; surface syntax later): `extend` → `Type.define(spec, …bundles)`; `interface` → `Interface.define(spec)`; named refinements (`where`) → `Refinement.define({refines: T, constraints: […]})`; **anonymous refinements keep the `&` operator as an R3 operator mint** — `Int & _ > 0` mints an anonymous Refinement instance exactly as `io & time` mints an anonymous Effect: ONE conjunction story across the type and effect worlds; `invariant` → additional `constraints` entries (already a list); `mixin` → bundle arguments; `preserveOps` → a Refinement spec option (operator-lifting policy); `distinct` → its own small sub-kind (restricts coercion/equality, not members) with the exact spec deferred to the C6.1b design; `~` untouched (projection, not construction). **(5) The constructor model**: `construct` is the STANDARDIZED per-kind minting-authority member (the D39 name `Type.construct`; the R2 capability — Type/Effect public, Proof kernel-private); **call-as-function invokes it at every level** — `Int(42)`, `Effect("net")` (already specified by D40 R4), `Type({…})` — one invocation rule; **named constructors are static factories**: ordinary own-members of the type value (data plane; found by the existing dispatch fallback) holding NO independent minting power — they DELEGATE to the type's `construct`; `Type.define` stands as Type's canonical named factory over `Type.construct` (a multi-field kind spec reads better behind `define`). **Every `construct` bottoms out in `struct_new` + the gated shape stamp**: `struct_new` alone yields an unshaped structure — the stamp is the mint; holding a kind's `construct` IS holding its mint, and Proof unforgeability becomes an ordinary capability instance (C6.3). |
| D46 | **MultiValue retirement = D15 execution; the definitional ladder; `kind` demoted to host discriminant (maintainer-ratified 2026-08).** With Phase 6 complete (Effect + Proof re-derived; M1 exit met), the MultiValue kind's remaining role is the channel carrier for non-Structure primaries (typed scalars, typed functions, residuals-with-components). Two retirement designs were evaluated: **(A) channel plane on leaf values** — every representation (Bits, functions, Expression, Param, Symbol) grows an optional plane; rejected: it saves no allocation (per-occurrence channels force copy-on-write cloning of immutable leaves — the carrier's cost by another name), smears the channel machinery (writers, merge policies, viral scan, W-invariants) from ONE hidden class across six object shapes, and re-opens the function-identity hazards (cloning ComposedFunctions breaks identity-keyed metadata); **(B) execute D15** — the transparent Structure (EMPTY data plane + `primary` CHANNEL) is the scalar carrier, "same construct, not a distinct wrapper kind" — **ratified; D15 reaffirmed unchanged**. The empty data plane is what keeps a carrier unmistakable for a record with no reserved member name; the "MultiValue interface" intuition survives as a HOST-LEVEL protocol (a value is representation + channel plane; non-Structure representations take the transparent carrier when channels attach), named in §2 but never reified in the kind tower — same reasoning that keeps Scope out of it. **The definitional ladder** (records the C6 vocabulary): host REPRESENTATIONS (`Bits`, `PrimitiveFunction`, `ComposedFunction`, and `Structure` the one composite — the ValueKind taxonomy, below the type system) → VALUES (representation + channels) → TYPES (language-level classifiers over representations: Int over Bits, `Function[P,R]` over functions, record types over Structures) → KINDS (types whose instances are type-values); `Scope` is on a parallel plane (D25 — shared substrate, own protocol, barred from dispatch and typing), neither type nor kind. None of Bits/Function/Structure/Scope is a Kind. **`v.kind` is demoted**: at the language level it does not exist (no primitive exposes it; `type of`, pattern matching, dataOf, and channel reads answer every observable question) — the language taxonomy is DATA vs COMPUTATION FORMS (Expression/Symbol/Param); at the host level the discriminant is irreducible (the evaluator must tell computation forms from self-evaluating data; TS narrows the tagged union) but it is an implementation detail, not spec — the "Seven Value Kinds" doc framing retires with the executing chunk. Execution is chunk C7.1. |
