...
- ✅ Provability arc — Phase H5 (PCP proof-library catalog — `allegro catalog`): the read-side counterpart to `extractObligations` — it enumerates a module's *discharged* theorems so a prover can CITE existing lemmas instead of reproving them. **`allegro catalog <file> [--json] [--output proofs.json]`** loads the file (`softFail` mode, same grammar/module pipeline as `inspect`/`verify`), walks `evalCtx` for discharged Proof bindings, and emits a `ProofCatalog`. Each `CatalogEntry` records: `name`, `proposition` (+ stable `propositionHash`), `dependencies` (identifiers referenced in the proposition, via `extractDependencies` — sorted, de-duplicated, keyword-filtered, dotted chains like `tactics.chain` kept whole), optional `refinementTypes` (dependency names that resolve to refined types — bindings carrying a `__predicate`), `strategy` (heuristic tags: `["auto-PE"]` for PE-discharged theorems with no `by` term; the constructors detected in the `by` term otherwise, via `classifyProofStrategy` — the canonical classifier the worker's `classifyStrategy` now delegates to), optional `proofTerm` (the source `by <term>` recovered by single-line regex), `authorship` (`auto-PE` for PE-discharged, `source` for `by`-term theorems whose prover can't be identified from source alone), and `location`. JSON is canonical (`serializeCatalog` pretty-prints; `parseCatalog` validates version + required fields); `--output` always writes JSON (the on-disk shape a worker reads); `formatCatalog` is the minimal plain-text render for direct CLI use. Retrieval helper `findCitableLemmas(catalog, target, {limit?})` returns entries sharing ≥1 dependency with a goal proposition (or pre-computed dep list), ranked by overlap count desc then name — the substrate for citation-instead-of-reproof. **Worker integration** (H5's payoff): `runLlmWorker` gains `catalogPath?` / `catalog?` options; when supplied it loads the catalog (malformed catalog is non-fatal) and, per obligation, merges the top-8 citable lemma names into the in-scope lemma list surfaced to the LLM via `buildIterationMessage`. CLI: `allegro prove --catalog proofs.json`. All new code lives in `src/pcp.ts` (schema + generator + retrieval + renderer) + additive CLI dispatch in `src/index.ts` + the worker wiring in `pcp/llm-worker.ts` — no grammar/parser/type-system changes. 8 new tests (7 sync: generator auto-PE + `by`-term recovery + dependency extraction + strategy classification + ranked retrieval + JSON round-trip/validation + refined-type tagging; 1 async: worker surfaces citable lemmas in the prompt); 986/986 green. Plan in `.claude/plans/phase-h-plan.md`. H6 (multi-strategy parallel orchestration) and H7 (effort budgets + reproducibility) remain — both quality/QoL increments; the H1+H2+H4 minimum that demonstrates the thesis bet shipped earlier.
