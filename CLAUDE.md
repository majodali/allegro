...
- **`src/grammar2/`** — New grammar formalism and parser (Phase 1 of the grammar overhaul per `docs/grammar-formalism.md`). Will eventually replace Pratt + Earley.
  ...
  - **`src/grammar2/engine.ts`** — Recursive scannerless parser with memoization, farthest-advance error reporting. Phase 1 scope: no indent terminals, no left recursion (added in Phase 2 via full GLL), no precedence (Phase 3 analyzer). Exports `parse(grammar, input)` → `ParseResult`.
  - **`src/grammar2/builder.ts`** — Allegro primitive wrappers (`grammar2_new`, `grammar2_lit`, `grammar2_cls`, `grammar2_regex`, `grammar2_seq`, `grammar2_alt`, `grammar2_rep`, `grammar2_opt`, `grammar2_nonterm`, `grammar2_guarded`, `grammar2_not_followed_by`, `grammar2_followed_by`, `grammar2_reserved`, `grammar2_indent`, `grammar2_add_production`, `grammar2_set_start`, `grammar2_reserved_set`, `grammar2_parse`). Rules/Grammars stored as opaque integer handles. `grammar2_parse` converts the engine's ParseTree into Allegro values (leaf → String, branch → Array or `{tag, children}` Object, none → noneSingleton, parse error → error MultiValue).
  - **`src/grammar2/base-grammar.ts`** — Allegretto base language as a Grammar value. Stratified precedence via layered productions (`expr_or` → `expr_and` → `expr_eq` → `expr_cmp` → `expr_add` → `expr_mul` → `expr_unary` → `expr_post` → `expr_atom`). Uses left recursion at each level via the engine's Warth iteration. Phase 2b scope: full base language minus blocks-as-values.
  - **`src/grammar2/tree-builder.ts`** — Converts grammar2 ParseTrees into Allegro Value trees (Expression, ComposedFunction, Symbol, Param, Bits). Mirrors the shape the existing hybrid parser emits, so downstream evaluation is unchanged. Covers: bindings, function declarations (typed + untyped), lambdas (all forms), if-then-else, when/is/then pattern matching with destructuring, binary/unary/type/keyword operators, calls, dot access, bracket indexing, number/float/string/bool/none/ident literals, parenthesized expressions, array/object literals with string interpolation, pipe operator, import/export statements, refinement types, structural wrap, blocks-as-values.

**Default parser:** Phase 2c-6 switched `evalSource` and the file runner to grammar2 as the primary path. Hybrid remains only for grammar-fragment handling (`use_grammar NAME` with register_* primitives) until grammar fragments are ported to grammar2 values.
