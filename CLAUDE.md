...
- ✅ Vivace models — domain-failure rendering hook (stage-(b) validation): the first chunk of the Project 1 planning-DSL pilot (`.claude/plans/project-1-planning-dsl-design.md`, priority #2). When a typed model object's construction or type check fails a constraint, the kernel produces a failure in *kernel* vocabulary ("refinement check failed: expected ≥ 1 (got -5)"); a model DSL needs it in its OWN vocabulary. A model attaches an Allegro renderer via `T.onFailure(renderer)` — a new `Type` meta-method (alongside `where`/`invariant`/`mixin`/…) that derives a type carrying a `__renderFailure` meta-binding. `src/types-std.ts` `buildFailureRendererType` CLONES the parent (never mutates — `Int.onFailure(...)` must not pollute the shared `Int`) and wraps `__construct` so the renderer participates regardless of declaration order: `(Int & pred).onFailure(R)` re-renders the enriched error the parent refinement constructor returns, while `Int.onFailure(R)` then `& pred` is inherited by the refinement (which copies `__renderFailure` forward) and renders directly. The kernel carries ZERO model knowledge — `tryRenderFailure(type, FailureComponents, ctx, evalFn)` looks up `__renderFailure`, and if present invokes it with a NON-error failure descriptor (typed `Object`, no `error` component — passing the raw Error value would trip the evaluator's error-propagation and the renderer would never run). The renderer reads the failure as data via component access: `message`/`actual`/`constraint`/`counterexample`/`failedType`/`failureKind`/`invariantIndex of f`. `makeFailureValue` enriches the actual Error value with the same fields under `__`-prefixed components (for introspection / future JSON surfaces). Wired into all stage-(b) failure sites: `refined.__construct` + `invariant.__construct` (error-value path, `src/types-std.ts`), `type_check_impl` (throw path, `src/primitives.ts`), and `checkArgType` (eager call-site path, `src/evaluator.ts`) — each falls back to the exact default kernel message when the type has no renderer, so behaviour is unchanged for all non-model code. Scope is stage (b) only (type checks + constraint proofs); stage-(c) custom analysis emits domain-legible output directly and gets its emission route in the `lib/planning.alg` chunk. Auto-derived renderers from the grammar definition are the long-term goal; for now they ship as part of the model definition (Allegro). Demo: `tests/render-hook-demo.alg`. 7 new tests; 985/985 green.
