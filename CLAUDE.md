...
- **`src/grammar2/`** — New grammar formalism and parser (Phase 1 of the grammar overhaul per `docs/grammar-formalism.md`). Will eventually replace Pratt + Earley.
  ...
  - **`src/grammar2/tree-builder.ts`** — Converts grammar2 ParseTrees into Allegro Value trees (Expression, ComposedFunction, Symbol, Param, Bits). Mirrors the shape the existing hybrid parser emits, so downstream evaluation is unchanged. Phase 2b covers: bindings, function declarations, lambdas, if-then-else, binary/unary operators, calls, number/string/ident literals, parenthesized expressions.
