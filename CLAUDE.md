...
- **`src/codegen/`** — Phase I code generation (target emission, the "Emitting" build phase). `resolve.ts` (`resolveProgram` — source → resolved pre-evaluation program, the front half of `evalSource` without the eval loop, so top-level side-effect order is preserved) and `js.ts` (`emitProgram` — syntax-directed JS/ESM backend; `CodegenError` on unmapped constructs). Consumes the resolved DAG; never modifies the evaluation pipeline. I1 landed (JS arithmetic/control core + `allegro emit`); I2-I6 in `.claude/plans/phase-i-plan.md`
- **`src/index.ts`** — Entry point: file runner + REPL. Allegro Standard by default, `--base` flag for base mode. On-demand module loading from `lib/` directory. Subcommands: `inspect`, `verify`, `obligations`, `propose`, `prove`, `emit`
...
- ✅ Provability arc — Phase I1 (code generation, JS/ESM backend substrate): the "Emitting" build phase, opening the codegen workstream (plan in `.claude/plans/phase-i-plan.md`, JS-first). Codegen lowers the resolved *pre-evaluation* program, NOT the post-eval residuals — `evalSource`'s eval loop fires and discards top-level side effects (a bare `print(x)`), so output order lives in the resolved `fileCtx.bindingList`, where source-to-source references stay as `Symbol(name)`, params as `Param`, primitives as their values. `src/codegen/resolve.ts` `resolveProgram(source, extensions, typed)` reproduces `evalSource`'s front half (parse → typeLiterals → resolveSymbols) without the eval loop. `src/codegen/js.ts` `emitProgram(program)` is a syntax-directed recursive `emitValue`: Int/Float/String literals (signed-64 two's-complement; Float disambiguated by type-component name), `ComposedFunction` → inline arrow or hoisted `function` decl for named bindings (self/mutual recursion, no TDZ), `eval_if` → ternary inlining 0-param branch thunks, arithmetic/comparison core (`bits_*` + `typed_*`) → JS operators, integer div → `Math.trunc`, `print` → inlined `__a` prelude shim, `Symbol`/`Param` calls → JS calls. Unmapped primitive or Context emission raises `CodegenError` naming the construct — no silent wrong output. CLI `allegro emit <file> [--out FILE.js] [--run]` error-gates by compiling exactly as `allegro run` would (refuse on effects/proof/proven throws; benign untyped-recursion precompile notices don't gate). `basics.alg` → idiomatic JS printing the exact pinned `11/42/120/42/55/42/7`. 10 behavioral-parity tests (emit → `node` → diff stdout; base mode + both CodegenError guards); 988/988 green. Separable from the concurrent planning-DSL workstream (only shared file `src/index.ts`, additive subcommand). I2 (typed values / dot dispatch / collections), I3 (corpus-parity harness), I4 (effect-/invariant-informed optimization — the Phase I headline), I5 (module emission), I6 (WASM) pending.
