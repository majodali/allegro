...
- **`lib/grammar-analyzer.alg`** — Phase 5 proof-of-concept: Allegro-native port of the static analyzer's simplest checks (defined-ness and reachability). Consumes the TS-built Grammar converted via `to-allegro.ts`, returns arrays of error/warning objects matching the TS analyzer's output. Parse-time on this module is significant (~40s on the current interpreter); amortized via module-level caching at test harness load. A full port of the remaining checks (nullability, FIRST, infinite-Rep, reservations, left-recursion) is deferred until the emitter (Phase 9) makes the performance viable.
